    ...
    def loadENCtrace(self, filename='', filepath='/GPFS/xf08id/pizza_box_data/'):
        ...
        with open(filepath + str(filename)) as f:
            for line in f:  # read rest of lines
                current_line = line.split()
                array_out.append(
                    [int(current_line[0]) + 1e-9 * int(current_line[1]), int(current_line[2]), int(current_line[3])])
        return np.array(array_out)
...
class XASdataAbs(XASdata):
    ...
    def interpolate(self):
        ...
        max_timestamp = np.array(
            [self.i0[len(self.i0) - 1, 0], self.it[len(self.it) - 1, 0], self.encoder[len(self.encoder) - 1, 0]]).min()
        ...
        self.energy_interp = np.array(
            [timestamps, np.interp(timestamps, self.energy[:, 0], self.energy[:, 1])]).transpose()
    ...
    def export_trace(self, filename, filepath='/GPFS/xf08id/Sandbox/', uid=''):
        ...
        if (not uid):
            ...
            pi, proposal, saf, comment, year, cycle, scan_id, real_uid, start_time, stop_time = db[uid]['start']['PI'], \
                                                                                                db[uid]['start'][
                                                                                                    'PROPOSAL'], \
                                                                                                db[uid]['start']['SAF'], \
                                                                                                db[uid]['start'][
        ...
        np.savetxt(fn, np.array([self.energy_interp[:, 0], self.energy_interp[:, 1],
                                 self.i0_interp[:, 1], self.it_interp[:, 1], self.ir_interp[:, 1]]).transpose(),
                   fmt='%17.6f %8.2f %f %f %f',
                   delimiter=" ", header='Timestamp (s)   En. (eV) 	i0 (V)	  it(V)	   ir(V)',
                   comments='# Year: {}\n# Cycle: {}\n# SAF: {}\n# PI: {}\n# PROPOSAL: {}\n# Scan ID: {}\n# UID: {}\n# Start time: {}\n# Stop time: {}\n# Total time: {}\n#\n# '.format(
                       year, cycle, saf, pi, proposal, scan_id, real_uid, human_start_time, human_stop_time,
                       human_duration))
        return fn
...
class XASdataFlu(XASdata):
    ...
    def export_trace(self, filename, filepath='/GPFS/xf08id/Sandbox/', uid=''):
        ...
        np.savetxt(fn, np.array([self.energy_interp[:, 0], self.energy_interp[:, 1],
                   ...
                   delimiter=" ", header='Timestamp (s)   En. (eV) 	i0 (V)	  it(V)',
                   comments='# Year: {}\n# Cycle: {}\n# SAF: {}\n# PI: {}\n# PROPOSAL: {}\n# Scan ID: {}\n# UID: {}\n# Start time: {}\n# Stop time: {}\n# Total time: {}\n#\n# '.format(
                       year, cycle, saf, pi, proposal, scan_id, real_uid, human_start_time, human_stop_time,
                       human_duration))
